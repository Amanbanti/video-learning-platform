  ...
  useEffect(() => {
    ...
    const fetchCourse = async () => {
      try {
        ...
        toast.error(err.response?.data?.message || err.message || "Failed to fetch course details")
      } finally {
        setLoading(false)
      }
    }
              ...
              <CardContent>
                <Accordion type="single" collapsible className="w-full">
                  {course.chapters.length > 0 ? course.chapters.map((chapter: any, index: number) => (
                    <AccordionItem key={chapter._id} value={chapter._id}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-left">
                              ...
                              <div className="text-sm text-muted-foreground flex items-center gap-2">
                                ...
                                <span className="text-sm text-muted-foreground pt-2">{chapter.duration} mins</span>
                              </div>
                            </div>
                          </div>
                        </div>
                      ...
                      <AccordionContent>
                        <div className="pt-2 pl-11">
                          <div className="text-sm text-muted-foreground pb-3">Description: {chapter.description}</div>
                          {activeChapter === chapter._id ? (
                            ...
                            <Button onClick={() => handleChapterSelect(chapter._id)} className="w-full sm:w-auto">
                              <Play className="h-4 w-4 mr-2 cursor-pointer" /> Watch Chapter
                            </Button>
                          )}
                        </div>
                      </AccordionContent>
                    ...
                    <div className="text-center text-muted-foreground col-span-3">No chapters found.</div>
                  )}
                </Accordion>
              </CardContent>
            </Card>
                ...
                <Dialog open={open} onOpenChange={setOpen}>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>Fill in the details below to add a new chapter to your course.</DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleAddChapter} className="space-y-4 mt-2">
                      <div className="pb-3">
                        ...
                        <Input placeholder="Chapter title" required value={title} onChange={(e) => setTitle(e.target.value)} />
                      </div>
                      <div className="pb-3">
                        <div className="pb-2 text-sm text-muted-foreground">Chapter Description</div>
                        <Textarea placeholder="Chapter description" required value={description} onChange={(e) => setDescription(e.target.value)} />
                      </div>
                      <div className="pb-3">
                        <div className="pb-2 text-sm text-muted-foreground">Chapter Video URL</div>
                        <Input placeholder="Chapter Video URL" required value={videoUrl} onChange={(e) => setVideoUrl(e.target.value)} />
                      </div>
                      <div className="pb-3">
                        <div className="pb-2 text-sm text-muted-foreground">Chapter Duration in minutes</div>
                        <Input placeholder="Chapter duration in minutes" value={duration} required onChange={(e) => setDuration(e.target.value)} />
                      </div>
              ...
              <CardContent className="space-y-4">
                <div className="text-sm text-muted-foreground">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span>Total Duration: {course.chapters.length > 0 ? course.chapters.reduce((acc, chapter) => acc + Number(chapter.duration), 0) + " mins" : "N/A"}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
